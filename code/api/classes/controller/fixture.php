...
class Controller_Fixture extends Controller_Hybrid
{
	...
	{
		return $this->response(Model_Matchcard::incompleteMatchcards(60, 7));
	}
	...
	public function put_index()
	{
		...
		if ($show != null) {
			...
			$card = Model_Matchcard::find_by_id($card['id']);
			if ($card['open'] > 1)
				return;
			if ($show === 'true')
				$card['open'] = 0;
	...
	public function action_repair()
	{
		...
		foreach (DB::query(/* SQL! */ "select id from matchcard where fixture_id is null and date > '2018-08-01'")->execute() as $row) {
			echo "Processing matchcard: {$row['id']}\n";
			$card = Model_Matchcard::find($row['id']);
		...
		echo "\nDuplicate cards\n";
		foreach (DB::query(/* SQL! */ "select fixture_id from matchcard group by fixture_id having count(*) > 1")->execute() as $row) {
			echo "{$row['fixture_id']}\n";
		}
	...
	public function get_summary()
	{
		...
		foreach (Model_Club::find('all') as $club)
			$codes[$club['name']] = $club['code'];

		foreach (Model_Fixture::getAll() as $fixture) {
			$x = $fixture['competition'];

			if (0 !== strpos($x, 'Division'))
				continue;

			if (!isset($result[$x]))
				$result[$x] = array('teams' => array(), 'fixtures' => array());

			$teams =& $result[$x]['teams'];

			if (!isset($codes[$fixture['home_club']]))
				continue;
			if (!isset($codes[$fixture['away_club']]))
				continue;
