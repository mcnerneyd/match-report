...
class Controller_Card extends Controller_Template
{
	...
	public function action_index() {
		...
		if ($cardId) {
			...
			if (\Auth::check()) {
				Response::redirect('/cards/index.php?site='.Session::get('site').'&controller=card&action=get&c0&fid='.$card['fixture_id']);
			} else {
				$data['card'] = $card;
			...
			Response::redirect('/cards/ui/?site='.Session::get('site'));
		}
	}
	...
	public function action_sendmail() {
		...
		*/
		...
		if ($msg != null) {
			...
			Response::redirect(rootUrl()."/card/index.php?controller=card&c2&action=index");
		}
