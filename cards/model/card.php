...
class Card
{
    ...
    public static function fixtures($club)
    {
      ...
      private static function fixtureFind($club, $fixtureId) // FIXME Messy
      {
          ...
          if ($fixtures) {
              foreach ($fixtures as $fixture) {
                  if ($fixtureId != null and $fixture->fixtureID != $fixtureId) {
                      continue;
                  }
      ...
      private static function convertFixture($fixture, $club, $comps)
      {
          ...
          if ($club != null) {
              if (!($home['club'] == $club or $away['club'] == $club)) {
                  return false;
              }
          }
      ...
      where m.id = :id";
        ...
        foreach ($req->fetchAll() as $row) {
            ...
            if ($row['type'] == 'Signed') {
                ...
                if (!isset($card[$side]['locked'])) {
                    $card[$side]['locked'] = 'Missing Code!';
                }
                continue;
            }
